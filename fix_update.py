...
def run(cmd_list, cwd=None, timeout=15):
    try:
        r = subprocess.run(
            cmd_list, capture_output=True, text=True, cwd=cwd, timeout=timeout
        )
...
with app.app_context():
    ...
    check(
        f"git rev-parse origin/master (exit={rc2})",
        rc2 == 0,
    ...
    if rc == 0 and rc2 == 0:
        ...
        if sha == origin:
            ...
            print(
                "  ⟹ The banner will NOT show until a NEW commit is pushed to GitHub."
            )
        else:
    ...
    if repo:
        for url_name, url in [
            (
                "VERSION file",
                f"https://raw.githubusercontent.com/{repo}/master/VERSION",
            ),
    ...
    if repo:
        ...
        try:
            r = httpx.get(
                f"https://raw.githubusercontent.com/{repo}/master/VERSION", timeout=10
            )
            ...
            check(
                "Version mismatch (would trigger banner)",
                version_update,
            ...
            try:
                ...
                commits_update = bool(
                    ...
                    and not version_update
                )
                check(
                    "Commit SHA mismatch (would trigger banner)",
                    commits_update,
    ...
    if not repo:
        ...
        Setting.set(
            "update_check_repo",
            "mail2jack/osint-dashboard",
            "GitHub repo for update checks (owner/repo)",
            "general",
        )
        repo = Setting.get("update_check_repo")
        check(
            "  Set update_check_repo → mail2jack/osint-dashboard",
            repo == "mail2jack/osint-dashboard",
        )
        fixes += 1
    ...
    if not can_show:
        ...
        print(
            "    echo '3.4.3' > VERSION && git add VERSION && git commit -m 'test: bump version' && git push"
        )
