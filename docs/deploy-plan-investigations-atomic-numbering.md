...
2. **Verifieer alleen na bevestiging dat het aparte DR-account (via `DR_VERIFY_DATABASE_URL` of `PGSERVICE`/`PGHOST`) én de DR-key/config (`DR_BACKUP_KEY_FILE`, standaard `backup-key.gpg` naast het archief) beschikbaar zijn.** `verify_backup.sh` neemt het concreet gekozen, bestaande archiefbestand (géén `--list`):
   ...
   >
   > **AFGEHANDELD 2026-08-30:** het aparte DR-account (`iveras_dr`) en de env bestaan sinds 2026-08-14 via `dr_setup.sh` (`/etc/default/osint-dr`, `PGSERVICE=iveras-dr`); de 17:42-fail van §10 kwam door het **ontbreken van die env** in de aanroep, niet door een ontbrekend account. `verify_backup.sh` wordt sindsdien structureel gevoed (auto-sources `/etc/default/osint-dr` als er geen expliciete verbindingsomgeving staat) en is groen bewezen op de nieuwste backup — zie de gesloten vervolgactie op de laatste regel van dit plan.
...
- **Backup + DR**: `backups/iveras_backup_20260829_174126.tar.gz.gpg`; `verify_backup.sh` faalde alleen op `database_restore` (DR-account niet geconfigureerd)**\*** → opgevangen via de in §7 toegestane lokale scratch-restore: `createdb osint_verify_20260829T174341Z` → `gunzip database.sql.gz | psql` → `alembic_version` = `bb1c2d3e4f5a7`, counts cases=103 / subjects=354 / investigations=0 / investigation_seq_counters=0 / case_number_counters=4, FORCE RLS `true` → `dropdb`. Restore-pad bevestigd. **\*** De oorzaak was het ontbreken van de DR-env in de aanroep; het account bestond sinds 14 aug (zie AFGEHANDELD-notitie bij §7). De structurele route is inmiddels groen: `reports/dr/dr-verification-20260830T074359Z.json` = 9/9 pass op `iveras_backup_20260830_060001`.
...
- **Open vervolgactie (na de rollout)**: `DR_VERIFY_DATABASE_URL`/`PGSERVICE`/`PGHOST` structureel invullen zodat `verify_backup.sh` blijvend groen is zonder scratch-restore.
  → **GESLOTEN 2026-08-30.** Bewijs: `/opt/osint-dashboard/reports/dr/dr-verification-20260830T074359Z.json` — **9/9 checks pass** op de nieuwste backup `iveras_backup_20260830_060001` (06:00 automatisch): `archive`, `extract`, `database_restore` (isoleer-DB `iveras_dr_…`, aangemaakt/gerestoreerd/gedropt door rol `iveras_dr`), `migration_version` = `a6b7c8d9e0f1` (huidige head), `row_counts` tenants=3 / cases=103 / users=5, `encrypted_fields` (Fernet-decrypt van een gerestaureerd veld), `license_database`, `license_private_key`, `uploads` — alle pass; géén achtergebleven isoleer-DB. De scratch-vervangingsroute uit §7 is daarmee vervallen (de 17:42-fail was een missende env in de aanroep). Ter voorkoming van herhaling: `verify_backup.sh` auto-sourcet `/etc/default/osint-dr` (beschikbaar via `dr_setup.sh`) en de periodic-verifier-units wijzen naar dezelfde env-file. Kandidaat-vervolgtap: de periodic-verifier (`deploy/osint-backup-verify.timer`) daadwerkelijk op de VPS installëren + alerting — apart te bekrachtigen.