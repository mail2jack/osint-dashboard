...
- `PREV_SHA` = waarde uit **`$PROJECT_DIR/.deployed_sha`** op de VPS (op dit
  moment `121c104`, na PR #145 — maar altijd uitlezen, niet hardcoden).
  `PREV_SHA`-als-ancestor van `TARGET_SHA` bewijst alleen dat de deploy forward
  is, **niet** dat PR #146 erin zit; het echte PR-#146-bewijs staat in stap 6.
...
1. App + health up; `$PROJECT_DIR/.deployed_sha` == `PREV_SHA`.
2. `git fetch origin` en `TARGET_SHA`-resolve + merge-freeze-bevestiging
   (zie 4); **bewijs dat `TARGET_SHA` PR #146 bevat**:
   `git merge-base --is-ancestor 1382deb "$TARGET_SHA"` (of: controleer dat
   migratie `f5a6b7c8d9e0` in die target aanwezig is, bv.
   `git cat-file -e "$TARGET_SHA:migrations/versions/f5a6b7c8d9e0_…py"`).
   `PREV_SHA`-als-ancestor bewijst dat niet — alleen de PR-#146-mergecommit/
   migratie doet dat. `alembic current` == `e2f3a4b5c6d7` (single head, pre).
