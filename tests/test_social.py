...
class TestSocialAccountCRUD:
    ...
    def test_add_social_account_requires_auth(self, client):
        resp = client.post(
            "/cms/api/subjects/1/social-accounts",
            json={
                "platform": "GitHub",
                "username": "testuser",
            },
    ...
    def test_add_social_account_invalid_subject(self, auth_client):
        resp = auth_client.post(
            "/cms/api/subjects/99999/social-accounts",
            json={
                "platform": "GitHub",
                "username": "testuser",
            },
    ...
    def test_delete_social_account_wrong_subject(self, auth_client, app):
        ...
        db.session.flush()
        account = SocialAccount(
            subject_id=s1.id, platform="github", username="testuser"
        )
        db.session.add(account)
        db.session.commit()

        resp = auth_client.delete(
            f"/cms/api/subjects/{s2.id}/social-accounts/{account.id}"
        )
        assert resp.status_code == 404
...
class TestExtractSocialId:
    ...
    def test_extract_social_id_with_extracted_data(self, auth_client, app):
        ...
        with patch(
            "cms.routes.social_extraction._extract_social_ids_from_url"
        ) as mock_extract:
