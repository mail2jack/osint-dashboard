...
class TestUsernameSearchRapidAPI:
    ...
    def test_requires_username(self, client):
        resp = client.post('/api/username/rapidapi', json={})
        assert resp.status_code == 400

    def test_empty_username(self, client):
        resp = client.post('/api/username/rapidapi', json={'username': ''})
        assert resp.status_code == 400

    def test_returns_usage_info_on_fallback(self, client):
        """Without API key, should return fallback_to_sherlock=True."""
        resp = client.post('/api/username/rapidapi', json={'username': 'testuser'})
        data = resp.get_json()
        assert data is not None
        assert 'api_usage' in data
        assert data.get('api_usage', {}).get('remaining') is not None

    def test_api_usage_structure(self, client):
        resp = client.post('/api/username/rapidapi', json={'username': 'testuser'})
        data = resp.get_json()
...
class TestSherlockFallback:
    ...
    def test_search_social_requires_auth(self, client):
        resp = client.post('/api/username/rapidapi', json={'username': 'testuser'})
        # This endpoint has no @login_required, so it should always respond
        assert resp.status_code in (200, 400, 429)

    def test_username_search_fallback_no_key(self, client):
