...
class TestFindingsCRUD:
    ...
    def test_create_finding_requires_auth(self, client):
        resp = client.post('/cms/findings/create', json={
            'case_id': 1,
            'subject_id': 1,
            'title': 'Test finding',
        })
    ...
    def test_check_existing_urls(self, auth_client, app):
        ...
        resp = auth_client.post('/cms/api/findings/check-existing-urls', json={
            'case_id': str(case.id),
...
class TestSocialFindings:
    ...
    def test_save_username_findings(self, auth_client, app):
        ...
        resp = auth_client.post(f'/cms/api/subjects/{subject.id}/save-username-findings', json={
            'case_id': str(case_id),
            'results': [
                {'url': 'https://github.com/testuser', 'platform': 'GitHub', 'username': 'testuser'},
                {'url': 'https://twitter.com/testuser', 'platform': 'Twitter', 'username': 'testuser'},
            ],
        })
        data = resp.get_json()
    ...
    def test_save_finding_as_social_account(self, auth_client, app):
        ...
        resp = auth_client.post('/cms/api/findings/save-as-social-account', json={
