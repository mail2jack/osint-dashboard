...
from . import cms_bp
from ..models import (
    db, Case, Subject, Finding, Comment,
    FinancialRecord, AuditLog, Document, Reminder
)
...
def export_case_json(case_id: str) -> flask.Response:
    ...
    export_data['findings'] = [f.to_dict() for f in Finding.query.filter_by(case_id=case_id).all()]
    export_data['documents'] = [d.to_dict() for d in Document.query.filter_by(case_id=case_id).all()]
    export_data['comments'] = [c.to_dict() for c in Comment.query.filter_by(case_id=case_id).all()]
    export_data['financials'] = [r.to_dict() for r in FinancialRecord.query.filter_by(case_id=case_id).all()]
    return flask.Response(
        json_mod.dumps(export_data, indent=2, default=str),
        mimetype='application/json',
        headers={'Content-Disposition': f'attachment; filename=case_{case.case_number}.json'}
    )
...
def case_report(case_id: str) -> str:
    ...
    findings = findings_q.options(db.joinedload(Finding.author)).order_by(Finding.created_at.asc()).all()
    comments = comments_q.options(db.joinedload(Comment.author)).order_by(Comment.created_at.asc()).all()
