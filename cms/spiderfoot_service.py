...
class SpiderFootService:
    ...
    def __init__(self, config: SpiderFootConfig | None = None):
        """
    ...
    def client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.error(
                    "spiderfoot-client not installed. Run: pip install spiderfoot-client"
                )
                return None
        return self._client
    ...
    def start_scan(
        ...
        if not scan_name:
            scan_name = (
                f"Iveras Scan - {target} - {datetime.now().strftime('%Y-%m-%d %H:%M')}"
            )
    ...
    def get_scan_results(
        self, scan_id: str, element_type: str | None = None, limit: int = 10000
    ) -> list[dict[str, Any]]:
        """
    ...
    def export_scan(self, scan_id: str, format: str = "json") -> str | None:
        """
    ...
    def map_to_iveras_finding(
        self, sf_result: dict[str, Any], case_id: str, subject_id: str | None = None
    ) -> dict[str, Any]:
        """
    ...
    def normalize_result(self, result: list) -> dict[str, Any]:
        ...
        if isinstance(result, list) and len(result) >= 3:
            ...
            return {
                ...
                "dataTransformed": source_url
                or (result[2] if len(result) > 2 else None),
...
_spiderfoot_service: SpiderFootService | None = None


def get_spiderfoot_service(config: SpiderFootConfig | None = None) -> SpiderFootService:
    """
...
def check_spiderfoot_health(sf_url: str | None = None) -> tuple[bool, str]:
    """
