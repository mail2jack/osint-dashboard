...
def spiderfoot_index():
    ...
    for sf_scan in sf_scans:
        if isinstance(sf_scan, list) and len(sf_scan) >= 7:
            ...
            if sf_id and sf_id not in db_sf_ids:
                ...
                mapped_status = {'finished': 'completed', 'error': 'failed', 'aborted': 'cancelled'}.get(api_status, api_status)
                recent_scans.append({
                    'id': sf_id,
                    'scan_id': sf_id,
    ...
    recent_scans.sort(key=lambda s: s.get('created_at', '') if isinstance(s, dict) else (s.created_at.strftime('%Y-%m-%d %H:%M:%S') if s.created_at else ''), reverse=True)
    ...
    status_map = {'finished': 'completed', 'running': 'running', 'pending': 'pending', 'failed': 'failed', 'error': 'failed', 'aborted': 'cancelled', 'cancelled': 'cancelled'}
    for s in sf_scans:
...
def spiderfoot_scan():
    ...
    if request.method == 'GET':
        # Show scan form
        cases = Case.query.filter_by(is_deleted=False).order_by(Case.case_number.desc()).all()
        subjects = Subject.query.filter_by(is_deleted=False).order_by(Subject.name).all()
    ...
    scan_record = SpiderFootScan(
        ...
        use_case=use_case,
        profile=profile,
        module_ids=SpiderFootService.INVESTIGATION_PROFILES.get(profile, {}).get('modules', []) if (SpiderFootService and profile) else [],
        status='pending',
        created_by=current_user.id
    )
    ...
    if sf_status:
        ...
        if not scan_record:
            scan_record = SpiderFootScan(
                id=scan_id,
                scan_id=sf_scan_id,
                scan_name=sf_status.get('scan_name', sf_status.get('title', f'Scan {sf_scan_id[:8]}')),
...
def spiderfoot_scan_results(scan_id: str):
    ...
    results = sf_service.get_scan_results(scan_record.scan_id, element_type=element_type, limit=limit)
