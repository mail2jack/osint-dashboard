...
(function() {
  function personTypeSelected(field) {
    var form = field.closest('form');
    if (!form) return false;
    var name = field.name || '';
    var prefix = name.replace(/_(?:name|voornamen|voorletters)$/, '');
    var type = form.querySelector('[name="' + prefix + '_type"]') ||
      form.querySelector('[name="subject_type"]');
    return !!type && type.value === 'person';
  }

  function relatedField(field, role) {
    var form = field.closest('form');
    if (!form) return null;
    var name = field.name || '';
    var prefix = name.replace(/_(?:name|voornamen|voorletters)$/, '');
    return form.querySelector('[name="' + prefix + '_' +
      (role === 'initials' ? 'voorletters' : 'voornamen') + '"]');
  }
  ...
  function normaliseGivenNames(value) {
    return value.trim().split(/\s+/).filter(Boolean).map(capitaliseFirst).join(' ');
  }
  ...
  document.addEventListener('input', function(event) {
    var field = event.target.closest('[data-person-name-role]');
    if (!field || !personTypeSelected(field)) return;
    var role = field.dataset.personNameRole;
    if (role === 'initials') {
      field.dataset.autoGeneratedInitials = 'false';
      return;
    }
    if (role === 'given-names') {
      var initialsField = relatedField(field, 'initials');
      if (initialsField && (!initialsField.value.trim() ||
          initialsField.dataset.autoGeneratedInitials === 'true')) {
        initialsField.value = initials(field.value);
        initialsField.dataset.autoGeneratedInitials = 'true';
      }
    }
  });
  ...
  document.addEventListener('blur', function(event) {
    var field = event.target.closest('[data-person-name-role]');
    if (!field || !personTypeSelected(field)) return;
    if (field.dataset.personNameRole === 'surname') {
      field.value = capitaliseFirst(field.value);
    } else if (field.dataset.personNameRole === 'given-names') {
      field.value = normaliseGivenNames(field.value);
      var initialsField = relatedField(field, 'initials');
      if (initialsField && initialsField.dataset.autoGeneratedInitials === 'true') {
        initialsField.value = initials(field.value);
      }
    }
  }, true);
