...
class SearchHistory:
    def __init__(
        self, history_file="search_history.json", archive_file="search_archive.json"
    ):
    ...
    def get_archive(self, limit=500, search_query=None, search_tool=None):
        ...
        if search_query:
            ...
            archive = [
                a
                for a in archive
                if query_lower in a.get("query", "").lower()
                or query_lower in a.get("results_summary", "").lower()
            ]
    ...
    def purge_old(self, days: int = 30) -> int:
        ...
        removed = 0
        for filepath in [self.history_file, self.archive_file]:
